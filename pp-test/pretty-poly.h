...
pp_rect_t pp_contour_bounds(const pp_contour_t c) {
  ...
  for(uint32_t i = 1u; i < c.point_count; i++) {
...
int32_t nodes[PP_MAX_INTERSECTIONS * 2][PP_MAX_INTERSECTIONS * 2];
uint32_t node_counts[PP_MAX_INTERSECTIONS * 2];
...
void build_nodes(const pp_contour_t contour, const pp_tile_t tile, pp_point_t origin, int scale) {
  ...
  for(uint32_t i = 0u; i < contour.point_count; i++) {
    pp_point_t point = {
...
void render_nodes(const pp_tile_t tile, pp_rect_t *bounds) {
  ...
  for(int32_t y = 0; y < (int32_t)node_buffer_size; y++) {
    if(node_counts[y] == 0) {
    ...
    for(uint32_t i = 0u; i < node_counts[y]; i += 2) {
      int sx = nodes[y][i + 0];
      int ex = nodes[y][i + 1];
